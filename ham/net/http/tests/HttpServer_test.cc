...
public:
    ...
    {
        server_.setHttpCallback(std::bind(&TestServer::handleRequest, this,
                                std::placeholders::_1,
                                std::placeholders::_2));
    }
...
private:
    ...
    {
        std::cout << "Method: " << request.methodToString() << std::endl
        ...
        if(request.path() == "/")
        {
            response.setStateCode(HttpResponse::StateCode::k200_Ok);
            response.setStateMessage("OK");
            response.setContentType("text/html");
            response.addHeader("Server", "Ham");

            std::string now = Timestamp::now().toFormatTime();
            response.setBody("<html><head><title>HamBurger</title></head>"
        "<body><h1>Hello!Welcome to HamBurger!</h1>Now is " + now +
        "</body></html>");
        }
        else if (request.path() == "/favicon.ico")
        {
            response.setStateCode(HttpResponse::k200_Ok);
            response.setStateMessage("OK");
        ...
        {
            response.setStateCode(HttpResponse::k404_NotFound);
...
int main()
{
    ...
    TestServer server(&loop, listenAddr, "HttpServer");
